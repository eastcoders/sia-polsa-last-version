...
class TestFeederConnection extends Command
{
     ...
     * @var string
     */
    protected $signature = 'test:feeder-connection {--clear : Clear cache before starting}';
     ...
     * @var string
     */
    protected $description = 'Test the connection and token logic of FeederApiService';

    /**
     * Execute the console command.
     */
    public function handle(FeederApiService $feederService)
    {
        $this->info('Starting Feeder API Connection Test...');

        if ($this->option('clear')) {
            $this->info('Clearing Feeder Token Cache...');
            Cache::forget('feeder_api_token');
        }

        try {
            // Test 1: Authenticate (Get Token)
            $this->info('Attempting Authentication...');
            $token = $feederService->authenticate();
            ...
            $token2 = $feederService->authenticate();
